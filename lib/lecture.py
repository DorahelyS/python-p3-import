...
Key Vocab
Module: a file containing Python definitions and statements. A module's functions, classes, and global variables can 
...
PyPI: the Python Package Index. A repository of Python packages that can be downloaded and made available to your 
application.
pip: the command line tool used to download packages from PyPI. pip is installed on your computer automatically when 
you download Python.
Virtual Environment: a collection of modules, packages, and scripts that can be activated or deactivated at any time.
Pipenv: a virtual environment tool that uses pip to manage the modules, packages, and scripts that you intend to 
...
Importing a Module
Lets use the Python os module as an example. We will use the name property to demonstrate the different ways to 
import.
...
>>> os.name
# => 'posix'
"posix", or the Portable Operating System Interface, is the name of the module that code is executed from in 
the Python shell.

A module may have multiple functions and definitions. What if we only want to use a few of them? A more concise 
way of importing is only importing the things we need. Lets say we want to import only the name property in the 
os module. We can do this by using the from keyword.
...
This can be bad practice, because it is less concise and you may run into namespace issues. (What if we had 
called another variable name?) When using the * operator we don't know exactly what we are taking from the module, 
...
Lets use the above file structure to practice. All the code below will be run in the Python REPL. 
Change directory into the absolute_package/ directory. To activate the REPL run python in the terminal.
...
>>> function1()
# => Function 1 in module 1
module6 is buried a bit deeper- to import its function1(), we need to crawl through subpackage1/ 
in package2:
...
Absolute imports remain valid even if the current location of the import statement changes and they make it 
easy to see exactly where the resource is coming from.
...
function1()
Now you can change directory to the parent directory of subpackage1. We can run module2 as a module using the following command.

$ python -m subpackage1.subpackage2.module2
# => Function 1 from module1

Relative Imports only work when we use them in a module. Which is why we need to include the -m argument. Relative imports cannot be invoked in scripts.
...
Relative imports are great if you have lots of code files that are related, but it can be unclear to other 
developers which modules are kept where. PEP-8 suggests that you only use relative imports when the absolute 
path extends past 79 characters, but it's a good tool to have at your disposal.
